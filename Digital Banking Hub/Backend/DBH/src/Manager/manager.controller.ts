import { BadRequestException, Body, Controller, Delete, Get, InternalServerErrorException, Param, Patch, Post, Put, Query, UseGuards, UsePipes, ValidationPipe } from "@nestjs/common";
...
export class ManagerController {
    constructor(private readonly managerService: ManagerService,
        ...
        async insertManagerDetails(@Body() data: ManagerDetails, @UploadedFile() picture: Express.Multer.File): Promise<Object> {
            try {
                const result = await this.managerService.findVerifiedManagerByEmailForAuth(data.Email);
                if (result == null) {
                    throw new BadRequestException("No Manager found associated with this email.");
                }
    
                const user = await this.managerService.findManagerDetailsByEmail(data.Email);
                if (user != false) {
                    throw new BadRequestException("Manager Details already exist.");
                }
    
                if (await this.managerService.insertManagerDetails(data, picture.filename)) {
