...
export class EmployeeController {
    ...
    @UsePipes(new ValidationPipe())
    async updateEmployee(@Param('userId') userId: string, @Body() myobj: EmployeeDTO, @UploadedFile() myfile: Express.Multer.File): Promise<Users | string> {
        const salt = await bcrypt.genSalt();
        const hashedpassword = await bcrypt.hash(myobj.password, salt);
        myobj.password = hashedpassword;
        myobj.filename = myfile.filename;
    ...
    passwordChange(@Body() myobj: changePasswordDTO, @Session() session): Promise<Authentication | string> {
        if (session && session['email']) {
            // Retrieve data from session
            const userEmail = session['email']; // Access the 'email' property stored in the session
